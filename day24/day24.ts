...
function and(objOne: string, objTwo: string) {
...
function or(objOne: string, objTwo: string) {
  if (values.get(objOne) == 1 || values.get(objTwo) == 1) {
    return 1;
  }
  return 0;
...
function xor(objOne: string, objTwo: string) {
  if (
    (values.get(objOne) == 0 && values.get(objTwo) == 1) ||
...
function part1() {
  ...
  let binary: string = "";
  collection.sort();
  for (let key = collection.length - 1; key >= 0; key--) {
    binary += values.get(collection[key])?.toString();
  }
